...
// Verifico que los campos enviados desde el formulario no esten vacios y quito espacios en blanco
	if(!empty(trim($_POST['form_usuario'])) && !empty(trim($_POST['form_email'])) && !empty($_POST['form_password'])){
		...
		$form_usuario = mysqli_real_escape_string($db, htmlspecialchars($_POST['form_usuario']));
		$form_email = mysqli_real_escape_string($db, htmlspecialchars($_POST['form_email']));
				...
				if (filter_var($form_email, FILTER_VALIDATE_EMAIL)) { 
						...
						$verifico_email = mysqli_query($db, "SELECT `email` FROM `usuarios` WHERE email = '$form_email'");
								...
								if(mysqli_num_rows($verifico_email) > 0){    
									$error_message = "El email ingresado ya se encuentra utilizado, utilice otro correo para registrarse.";
										...
										$inserto_usuario = mysqli_query($db, "INSERT INTO `usuarios` (usuario, email, password) VALUES ('$form_usuario', '$form_email', '$usuario_hash_password')");
